...
const EPIPage: React.FC = () => {
  ...
  const addItem = () => {
    setItems(prev => [...prev, { tipo: EPI_TYPES[0], descricao: '', ca: '', tamanho: '', quantidade: 1, observacao: '' }]);
  };
    ...
    <div className="space-y-5 animate-fade-in">
      <div className="card-premium p-6 gradient-primary text-primary-foreground">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold font-display">Entrega de EPI</h1>
            <p className="text-primary-foreground/70 text-sm">Controle de entrega de Equipamentos de Proteção Individual</p>
          </div>
        </div>
      </div>
      ...
      <div className="card-premium p-5 space-y-4">
        ...
        {search && !selectedEmpId && (
          <div className="border rounded-lg max-h-48 overflow-y-auto">
            ...
            {filteredEmps.length === 0 && <p className="p-3 text-sm text-muted-foreground">Nenhum funcionário encontrado</p>}
          </div>
        )}
        ...
        {emp && company && (
          <div className="bg-muted/30 rounded-lg p-4 grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
            ...
            <div className="flex items-end">
              <Button variant="ghost" size="sm" onClick={() => setSelectedEmpId('')} className="text-xs text-destructive">Trocar funcionário</Button>
            </div>
          </div>
        )}
      </div>
      ...
      {emp && (
        <div className="card-premium p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold text-foreground">Itens de EPI</h2>
            <Button size="sm" onClick={addItem}><Plus className="w-4 h-4 mr-1" />Adicionar Item</Button>
          </div>

          {items.map((item, idx) => (
            <div key={idx} className="border rounded-lg p-3 grid grid-cols-2 md:grid-cols-6 gap-2 items-end">
              ...
              <div>
                ...
                <Input value={item.descricao} onChange={e => updateItem(idx, { descricao: e.target.value })} className="h-8 text-xs" placeholder="Detalhe..." />
              </div>
              <div>
                <label className="text-[10px] text-muted-foreground">CA</label>
                <Input value={item.ca || ''} onChange={e => updateItem(idx, { ca: e.target.value })} className="h-8 text-xs" placeholder="Opcional" />
              </div>
              <div>
