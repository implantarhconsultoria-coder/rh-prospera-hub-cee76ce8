-- Add CPF columns to profiles
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS cpf text,
  ADD COLUMN IF NOT EXISTS cpf_clean text;

CREATE INDEX IF NOT EXISTS idx_profiles_cpf_clean ON public.profiles(cpf_clean);

-- Trigger to auto-fill cpf_clean from cpf
...
BEGIN
  IF NEW.cpf IS NOT NULL THEN
    NEW.cpf_clean := regexp_replace(NEW.cpf, '[^0-9]', '', 'g');
    IF NEW.cpf_clean = '' THEN NEW.cpf_clean := NULL; END IF;
  ELSE
    NEW.cpf_clean := NULL;
  END IF;
  RETURN NEW;
...
-- Allow admins to update any profile (for CPF editing in Gerenciar Usuários)
...
-- Update CPF validation to also recognize profiles.cpf as a valid source
-- (keeps funcionarios as primary; falls back to profiles when CPF not in funcionarios)
...
BEGIN
  ...
  -- 2) Se não achou em funcionarios, tenta profiles (admin/usuário do sistema com CPF)
  IF NOT FOUND THEN
    ...
    IF FOUND THEN
      -- profile existe; admin tem acesso total
      IF EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = v_profile.user_id AND role = 'admin') THEN
