...
-- 1) Expandir status permitidos no fechamento da filial
ALTER TABLE public.fechamentos_filial DROP CONSTRAINT IF EXISTS fechamentos_filial_status_check;
ALTER TABLE public.fechamentos_filial ADD CONSTRAINT fechamentos_filial_status_check
  CHECK (status = ANY (ARRAY['pendente','aberto','em_andamento','fechado','reaberto']));

-- 2) Atualizar validar_acesso_cpf_slug para reconhecer slugs novos
--    e auto-criar permissão quando o Admin libera depois (fallback amplo).
...
BEGIN
  ...
  -- 2) Se não achou em funcionarios, tenta profiles (admin via CPF)
  IF NOT FOUND THEN
  ...
  -- Tenta módulo específico (área_filial), depois área genérica
